...
SELECT e2.puesto, e1.nombre, e1.apellido1, e1.apellido2, e1.email
FROM empleado AS e1
INNER JOIN empleado AS e2 ON e1.codigo_empleado = e2.codigo_jefe
WHERE e2.codigo_empleado = 7;
...
-- 6 Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante de ventas.
SELECT c.nombre_cliente, e.nombre, e.apellido1
FROM cliente AS c
INNER JOIN empleado AS e ON c.codigo_empleado_rep_ventas = e.codigo_empleado;

-- 7 Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT c.nombre_cliente, e.nombre, e.apellido1
FROM cliente AS c
INNER JOIN empleado AS e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
WHERE c.codigo_cliente IN (SELECT DISTINCT codigo_cliente FROM pago);

-- 8 Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de 
...
-- 9 Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus 
...
-- 10 Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus representantes 
...
-- 11 Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.
SELECT DISTINCT o.linea_direccion1, o.linea_direccion2, o.ciudad
FROM oficina AS o
WHERE o.codigo_oficina IN (SELECT DISTINCT codigo_oficina FROM cliente WHERE ciudad = 'Fuenlabrada');

-- 12 Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT c.nombre_cliente, e.nombre, e.apellido1, o.ciudad
FROM cliente AS c
INNER JOIN empleado AS e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
INNER JOIN oficina AS o ON e.codigo_oficina = o.codigo_oficina;

-- 13 Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
SELECT e.nombre AS empleado_nombre, e.apellido1 AS empleado_apellido, e.jefe_nombre, e.jefe_apellido
FROM (
  SELECT e1.nombre, e1.apellido1, e2.nombre AS jefe_nombre, e2.apellido1 AS jefe_apellido
  FROM empleado AS e1
  LEFT JOIN empleado AS e2 ON e1.codigo_jefe = e2.codigo_empleado
) AS e;

-- 14 Devuelve un listado que muestre el nombre de cada empleados, el nombre de su jefe y el nombre del jefe de sus jefe.
SELECT e.nombre AS empleado_nombre, e.apellido1 AS empleado_apellido, e.jefe_nombre, e.jefe_apellido, e.jefe_del_jefe_nombre, e.jefe_del_jefe_apellido
FROM (
  SELECT e1.nombre, e1.apellido1, e2.nombre AS jefe_nombre, e2.apellido1 AS jefe_apellido, e3.nombre AS jefe_del_jefe_nombre, e3.apellido1 AS jefe_del_jefe_apellido
  FROM empleado AS e1
  LEFT JOIN empleado AS e2 ON e1.codigo_jefe = e2.codigo_empleado
  LEFT JOIN empleado AS e3 ON e2.codigo_jefe = e3.codigo_empleado
) AS e;

-- 15 Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un pedido.
...
SELECT e1.codigo_empleado, e1.nombre, e1.apellido1, e1.apellido2, e1.email, e1.codigo_jefe, e2.nombre AS nombre_jefe, e2.apellido1 AS apellido_jefe
FROM empleado AS e1
LEFT JOIN empleado AS e2 ON e1.codigo_jefe = e2.codigo_empleado;
...
-- 23 Calcula el número de clientes que tiene la empresa.
SELECT COUNT(*) AS cantidad_clientes
FROM cliente;

-- 24 ¿Cuántos clientes existen con domicilio en la ciudad de Madrid?
SELECT COUNT(*) AS cantidad_clientes_madrid
FROM cliente
WHERE ciudad = 'Madrid';

-- 25 ¿Calcula cuántos clientes tiene cada una de las ciudades que empiezan por M?
SELECT LEFT(ciudad, 1) AS letra, COUNT(*) AS cantidad_clientes
FROM cliente
WHERE LEFT(ciudad, 1) = 'M'
GROUP BY letra;

-- 26 Devuelve el nombre de los representantes de ventas y el número de clientes al 
-- que atiende cada uno.
SELECT e.nombre, e.apellido1, COUNT(c.codigo_cliente) AS cantidad_clientes_atendidos
FROM empleado AS e
LEFT JOIN cliente AS c ON e.codigo_empleado = c.codigo_empleado_rep_ventas
GROUP BY e.nombre, e.apellido1;

-- 27 Calcula el número de clientes que no tiene asignado representante de ventas.
SELECT COUNT(*) AS cantidad_clientes_sin_representante
FROM cliente
WHERE codigo_empleado_rep_ventas IS NULL;

-- 28 Calcula la fecha del primer y último pago realizado por cada uno de los clientes. 
...
SELECT codigo_pedido, COUNT(*) AS cantidad_productos_diferentes
FROM detalle_pedido
GROUP BY codigo_pedido;

-- 30 Calcula la suma de la cantidad total de todos los productos que aparecen en 
-- cada uno de los pedidos.
SELECT codigo_pedido, SUM(cantidad) AS cantidad_total_productos
FROM detalle_pedido
GROUP BY codigo_pedido;

-- 31 Devuelve un listado de los 20 productos más vendidos y el número total de unidades 
-- que se han vendido de cada uno. El listado deberá estar ordenado por el número total 
...
-- 32 La facturación que ha tenido la empresa en toda la historia, indicando la base imponible, 
-- el IVA y el total facturado. La base imponible se calcula sumando el coste del 
-- producto por el número de unidades vendidas de la tabla detalle_pedido. 
...
-- 33 Devuelve el listado de clientes indicando el nombre del cliente y cuántos 
...
-- 34 Devuelve un listado con los nombres de los clientes y el total pagado por cada uno de ellos. 
...
-- 35 Devuelve el nombre de los clientes que hayan hecho pedidos en 2008 ordenados 
...
-- 36 Devuelve el nombre del cliente, el nombre y primer apellido de su representante de ventas 
...
-- 37 Devuelve el listado de clientes donde aparezca el nombre del cliente, 
...
-- 38 Devuelve el nombre, apellidos, puesto y teléfono de la oficina de aquellos empleados que 
...
-- 39 Devuelve un listado indicando todas las ciudades donde hay oficinas y el número de empleados que tiene.
SELECT o.ciudad, COUNT(e.codigo_empleado) AS numero_empleados
FROM oficina AS o
LEFT JOIN empleado AS e ON o.codigo_oficina = e.codigo_oficina
GROUP BY o.ciudad;

-- 40 Devuelve el nombre del producto del que se han vendido más unidades. 
-- (Tenga en cuenta que tendrá que calcular cuál es el número total de unidades que se 
...
INNER JOIN (
  ...
  ORDER BY total_unidades_vendidas DESC
  LIMIT 1
) AS subquery ON p.codigo_producto = subquery.codigo_producto;
